...
public class PulsarConnectionFactory
  ...
  public ConsumerBase<?> createConsumer(
    ...
    try {
      ...
      if (consumerConfiguration.getConsumerConfiguration().containsKey("messageCrypto")) {
        builder.messageCrypto(
            (MessageCrypto)
                consumerConfiguration.getConsumerConfiguration().get("messageCrypto"));
      }
      if (enablePriority) {
